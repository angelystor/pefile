	...
	for idx := uint32(0); idx < uint32(len(table)); idx++ {
		...
		if table[idx].Data.AddressOfData > 0 {
			...
			if table[idx].Data.AddressOfData & ordinalFlag > 0 {
				...
				section := self.getSectionByRva(imp.HintNameTableRva)
				offset := imp.HintNameTableRva - section.Data.VirtualAddress + section.Data.PointerToRawData
				if err := self.parseHeader(&imp.Hint, offset, 2); err != nil {
					return err
				}
